...
public class MainActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        data=new ArrayList<>();
        add("绘制形体(S)",FGLViewActivity.class);
        add("图片处理(F)",SGLViewActivity.class);
    ...
    private void onCheckPermission() {
        if (!hasPermissionsGranted(VIDEO_PERMISSIONS)) {
            requestPermissions(VIDEO_PERMISSIONS, 1024);
        }
    }
    ...
    public void onClick(View view){
        ...
        if(bean.clazz.equals(FGLViewActivity.class)){
            startService(new Intent(this, BackRecordService.class));
        }else if(bean.clazz.equals(SGLViewActivity.class)){
            EventBus.getDefault().post(new BackRecordService.StopEvent());
        }
