package dev.ngai.fantastic.sharedpres;
...
public class SharedPres {
    ...
    static public SharedPreferences getInstance(String name) {
        ...
        SharedPreferences tSp = mSharedPrefs.get(name);
        if (tSp == null) {
            tSp = Appfantastic.getContext().getSharedPreferences(name, Context.MODE_PRIVATE);
            mSharedPrefs.put(name, tSp);
        }
        return tSp;
    }

    static private HashMap<String, SharedPreferences> mSharedPrefs;
